...
public class Contact extends Resource {
	...
	public void parseEntity(InputStream is) throws IOException, VCardException {
		...
		if (uid != null) {
			...
			Log.getInstance().warn( "Received VCard without UID, generating new one");
			generateUID();
		}
		...
		if (fn != null) {
			...
			Log.getInstance().warn( "Received invalid VCard without FN (formatted name) property");
	...
	public ByteArrayOutputStream toEntity() throws IOException {
		...
		try {
			...
			Log.getInstance().warn( "Couldn't parse original property set, beginning from scratch");
		}
		if (vcard == null)
			vcard = new VCard();
		...
		if (uid != null)
			...
			Log.getInstance().error( "Generating VCard without UID");
		...
		if (displayName != null)
			...
			Log.getInstance().warn( "No FN (formatted name) available to generate VCard");
		...
		// PRODID, REV
		vcard.setProdId("DAVdroid/" + Constants.VERSION + " (ez-vcard/" + Ezvcard.VERSION + ")");
